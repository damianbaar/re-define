var _ = require('underscore')
  , ast = require('../ast/ast-adapter')

module.exports = {
  match     : match
, attach    : attach
}

function match(block) { 
  return ast.is.amd('require', block()[0]) && block().length === 1
}

function attach(mod, code) {
  var module = mod
    , body = code()
    , args = ast.get.expressionArgs(body[0])

  return {
    deps      : deps()
  , params    : params()
  , transform : transform 
  , type      : 'amd-require'
  , variable  : true
  }

  function func() {
    if(args.length === 1) return args[0]
    if(args.length === 2) return args[1]
  }

  function deps () {
    var deps

    if(args.length === 2) deps = args[1].elements
    else if(args.length === 1) deps = args[0].elements

    return _(deps).map(function(e) { return e.value })
  }

  function params() {
    return ast.is.func(func()) 
           ? _.map(func().params, function(e) { return e.name })
           : []
  }

  function transform() {
    var deps = module.deps
      , f = func()
      , create = ast.create
      , args = _(module.deps).map(function(d) { return create.id(d) })
    
    f.params = _(module.params).map(function(d) { return create.id(d) })

    return ast.generate(create.expressionStatement(create.callExpression(args, f)))
  }
}


...
function start(config, transform) {
  ...
  var print = through(options, function(files, enc, next) {
    this.push(_.map(files, function(modules, bundle) { 
      var intro = '\n---------'+bundle+'-----------\n'

      return intro + _.map(modules, function(m) { 
        var external = !_.has(m, 'transform')
        return external ? m : '\n--' + m.name + '--\n' + m.transform() + '\n' })
    }).join('\n'))
    next()
  }, function(end) {
  ...
  return combiner([stream, api.transform.sort(config), print])

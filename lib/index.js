...
function bundle(config, transform) {
  ...
  converter = through(options, function(file, enc, next) {
    ...
    var pending = _.find(files, function(p) { return p.path === file.path })

    if(pending && pending.exists) {
      pending.related = pending.related || []
      pending.related.push(file)
  ...
  return combiner(
    [ stream
    //TODO there needs to be strategy for it 
    , combiner([ sort(config)
               , convertAST(config)
               , wrap(config) ]) 
    ])

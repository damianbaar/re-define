...
function start(config, transform) {
  ...
  converter
      ...
      if(file.exists) { 
        ...
        function _end(location) {
          ...
          if(!!location && !!location.path) {
            stream.write(
              _.defaults(new File(
                {base: path.dirname(file.path), path: location.path })
    ...
    .pipe(through(options, function(file, enc, next) {
      ...
      _.each(file.deps, function(d) {
        stream.write(_.merge(new File({ base: file.base }), d, {parent: file.parent}))
      })
  ...
  stream = through(options, function(chunk, enc, next) {
    ...
    this.push(_(data).groupBy('parent').map(function(modules, module) { 
      ...
      return intro + _.map(modules, function(m) { return m.transform() })
    }).join('\n'))
    end()
  })

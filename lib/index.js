  ...
  converter
    ...
    .pipe(through(options, function(file, enc, next) {
      if(file.exists) { 
        ...
        var discoverable = config.discoverableDirs
        ...
        likelyLocation = 
          _(discoverable)
            .map(function(d) {
              return [ path.resolve(d, appendJS(file.orig))
        ...
        async.detect(likelyLocation, fs.exists, _end)

        function _end(location) {
          location = _.isString(location) ? {path: location } : location

          if(!!location && !!location.path) 
            stream.write(_.merge(new File({base: path.dirname(file.path), path: location.path })))
          else
    ...
    .pipe(through(options, function(file, enc, next) {
      _.each(file.deps, function(d) {
        d.parent = file.parent || file.name
        stream.write(_.merge(new File({ base: file.base }), d))
      })

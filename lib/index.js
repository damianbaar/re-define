...
function start(config, transform) {
  ...
  stream = through(options, function(chunk, enc, next) {
    if(_.isString(chunk)) chunk = new File({cwd: config.base, orig: 'main', path: chunk})

    converter.write(chunk)
    next()
  }, function(end) {
    this.push(_.merge(_.groupBy(data, 'parent'), {external: external}))
    end()
  })

  return combiner(
    [ stream
    , api.transform.sort(config)
    , api.transform.wrap(config)])

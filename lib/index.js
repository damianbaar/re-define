...
var loadFile        = require('./transform/load')
  ...
  , adapter         = require('./transform/browserify-adapter')
  ...
  stream = through(options, function(file, enc, next) {
    ...
    if(paths.indexOf(file.path) === -1) {
      ...
      if(processing) {
        ...
        //align file when using glob **, first we've got something with content then reference to it

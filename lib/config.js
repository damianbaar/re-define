...
module.exports = function (userConfig) {
  ...
  if(_wrapper && !config.wrappers[_wrapper]) {
    var w = coreWrappers[_wrapper]
    config.wrappers[_wrapper] = _.isFunction(w) ? w() : w
  }
  ...
  config.version = desc.version

  config.requestChange = function(property, val) {
    var checkDefault = function(property) {
        return userConfig[property] && join(userConfig[property]) != join(config[property])
        function join(arr) { return (arr || []).join() }
    }

    if(checkDefault(property) && _c.showWarnings) {
      console.log( '//conflict in confg for', property, ' field,'
                 , 'globalConfig and one of the transform contain different key configuration,'
                 , 'global', userConfig[property], val, ','
                 , 'taking from transform.')
    }

    _c[property] = val
  }

  var _c = _.merge(config, userConfig, function(a,b) {

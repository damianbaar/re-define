...
module.exports = function(config) {
  ...
  function onModule(module, next, until) {
    ...
    converter(module, modules, function(m) {
      ...
      if(m.name && !module.name) module.name = m.name
    ...
    resolver(module, modules, function (d) {
      ...
      _.each(external, function(name) { !modules[name] && registerModule({name: name, type: 'external'}) })
      _.each(toLoad, function(name) { registerModule({name: name}) })

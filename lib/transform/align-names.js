...
module.exports = function(config) {
  ...
  return through.obj(function(file, enc, next) {
    ...
    if(!file.parent) file.parent = goAwayWin(removeExt(file.relative))
    if(!file.name) file.name = goAwayWin(removeExt(file.relative))
    if(!file.require) file.require = goAwayWin(file.name)
    ...
    _.each(file.deps, function(d) {
      ...
      var _plug = _.find(config.plugins, function(d) { return new RegExp(d.pattern).test(pluginName) })
        ...
        , _name = goAwayWin(_plug ? pluginName + './' + _rel : removeExt(_rel))
    ...
    function goAwayWin(name) { return name.replace(/\\/g,'/') }

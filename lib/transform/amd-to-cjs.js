    ...
    if(n.ObjectExpression.check(fun)) {
      var a = b.expressionStatement( 
                b.assignmentExpression(
                  '=', b.identifier(config.exportVar), fun))
    ...
    if(fun) {
      walk.ancestor(fun, {
        ReturnStatement: function(node, parents) {
          if(parents.length === 3 && n.BlockStatement.check(parents[1])) {
            var owner = parents[1]
              , idx = owner.body.indexOf(node)

            if(idx === -1) return

            owner.body[idx] = b.expressionStatement( 
                                b.assignmentExpression(
                                  '=', b.identifier(config.exportVar) , node.argument))
          }
        }
      })
    }
    
    //TODO consider stopProcessing when module/exports/require does not exists as dep
    if(!_.isEmpty(deps)) {
      deps = _(deps)
             .map(function(e, i) { 

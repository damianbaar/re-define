    ...
    _(files)
      ...
      .each(function(f) {
        var folders = config.discoverable
          ...
          , name = f.name
          , considerAsIternal = !_.isEmpty(config.base) 
              ? path.dirname(path.join(f.base, f.name)) == base
              : false

        if(!_.has(f,'external') && !considerAsIternal) {
          _.each(folders, function(folder) {
            if(safePath.lastIndexOf(folder) > -1) {
              f.external = true
            }
          })
        }
      ...
      .each(function(group, base) {
        ...
        if(isExternal) {
          ...
          _.each(group, function(f) {
            //when group contains only one file, then it is safe to rewrite name appropriately to package.json.main
            if(f === fromDescriptor && fromDescriptor.descriptor && group.length < 2) {
              f.name = f.packageName = fromDescriptor.pkgName
              return
            }

            if(fromDescriptor && fromDescriptor.external)
              pkg = f.pkgName || fromDescriptor.pkgName

            var moduleName = f.name.split('/')
              , folder = moduleName.shift()

            //TODO investigate pkg
            f.name = folder ? folder + '/' + moduleName.join('/') : f.name
    ...
    _(files)
      ...
      .each(function(files, path) {
        ...
        _.each(files, function(f) {
          ...
          debug('Changing module name from %s to %s', f.requiredAs, f.name)

    ...
    return through.obj(function(files, enc, next) {
      ...
      async.waterfall([
        ...
        function(cacheEntries, cb) {
          ...
          var toRemove = _(files).map(function(f) {
        ...
        function(cb) {
          async.each(files, function(file, callback) {
            ...
            var _cache = cachedFilename(file)
            fs.writeFile(_cache, stringifyFile(file, _cache)
            , function(err) {
              if (err) throw err

              debug('Caching file to %s %s', file.name, _cache)
              callback()
            })
          }, cb)
        }

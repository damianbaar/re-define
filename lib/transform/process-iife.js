...
module.exports = function(config, writer) {
  return through.obj(function(file, enc, next) {
    ...
    walk.recursive(file.contents, state, {
      CallExpression: function(node, st, c) {
        if(n.FunctionExpression.check(node.callee)) {
          ...
          node.arguments = _.map(node.arguments, function(a) {
            if(n.ThisExpression.check(a))
              return b.identifier('window')
      ...
      AssignmentExpression: function(node, st, c) {
        if(st.iife) {
          var left = node.left
            , right = node.right
            , callee = n.Literal.check(right) ? right.raw : right.callee.name
            , to = left.object && left.object.name
            , deps = []

          console.log(right)

          var global = st.params.indexOf(to) > -1 && st.params.indexOf(callee) > -1
            , cjs = (callee && st.params.indexOf(callee) > -1) 
    ...
    if(state.iife) {
      ...
      file.deps = _.map(state.deps, function(d) {
        var dep = {}
      ...
      file.stopProcessing = file.stopProcessing
      file.wrap = false

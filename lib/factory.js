...
function introduceVar(body, configuration, filePath) {
  ...
  _(deps).each(function(d){ d.value = normalizePath(d.value, filePath) })

  name = applyShim(name)

  variable = isComplexPath(name) 
             ? name
             : "var " + name 
...
function wrap(body, config) {
  ...
  _(config.globals).each(function(d) {
    args.push(!!d.global ? d.global + squareBrackets(d.lib) : d.lib)
    params.push(!!d.as ? d.as : d.lib)
  })

  //TODO initializeglobals
  ...
  return wrapperBody.body = block, wrapper
}
...
function normalizePath(name, filePath) {
  ...
  if(isPlugin(name)) {
    var pluginPath = name.replace(/^(\w*[\///]?)*!/, "")
      , pluginName = name.replace(pluginPath, "")

    return pluginName + normalize(pluginPath, filePath)
  }
